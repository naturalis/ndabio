...
function setTaxonSpecimenLink (request, total) {
	...
	if (total > 0) {
		...
		jQuery('#taxon_specimens').html('<a href="?nba_request=' + request + '">' + label + ' (' + total + ')</a>');
	} else {
...
function setTaxonMultimediaLink (request, total) {
	total = parseInt(total);
	if (total > 0) {
		jQuery('#taxon_multimedia').html('<a href="?nba_request=' + request + '">Multimedia (' + total + ')</a>');
	} else {
...
function setSpecimenMultimediaLink (request, total) {
	total = parseInt(total);
	if (total > 0) {
		jQuery('#specimen_multimedia').html('<a href="?nba_request=' + request + '">Multimedia (' + total + ')</a>');
	} else {
