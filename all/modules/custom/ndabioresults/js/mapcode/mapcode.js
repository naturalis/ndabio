...
function encode_grid(m,y,x,codex,minx,miny,maxx,maxy,pipe,ccode)
{
  ...
  var relx = x-minx;
  if (relx<0) { x+=360000000; relx+=360000000; } else if (relx>=360000000) { x-=360000000; relx-=360000000; } // 1.32 fix FIJI edge case
  if (relx<0) 
    return NaN;
		...
		if ( pipetype==0 && isnameless==0 && codexlen==incodexlen && codexhi==incodexhi ) {
			...
			if ( isuseless && result ) {
				...
				var j;for (j=upto-1;j>=from;j--) { // look in previous rects
					...
					var xdiv8 = x_divider(miny,maxy)/4; // 1.32 allow extra room, just like for all other results
					if ( miny-45<=result.y && result.y<maxy+45 && isInRange(result.x,minx-xdiv8,maxx+xdiv8) ) { fitssomewhere=1; break; }
				}
